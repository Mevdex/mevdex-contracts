...
import "./IMevdexPair.sol";

contract MevdexChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Mev to distribute per block.
        uint256 lastRewardTimestamp; // Last block number that Mev distribution occurs.
        uint256 accMevPerShare; // Accumulated Mev per share, times 1e18. See below.
    ...
    IBoringERC20 public mev;

    // Mev tokens created per second
    uint256 public mevPerSec;
    ...
    // Update the given pool's Mev allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
        ...
        )
    {
        ...
        uint256 accMevPerShare = pool.accMevPerShare;
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accMevPerShare += (
                ((mevReward * ACC_TOKEN_PRECISION) / lpSupply)
            );
        }

        uint256 pendingMev = (((user.amount * accMevPerShare) /
        ...
        addresses[0] = address(mev);
        symbols[0] = IBoringERC20(mev).safeSymbol();
        decimals[0] = IBoringERC20(mev).safeDecimals();
        amounts[0] = pendingMev;
        ...
        )
    {
        ...
        rewardsPerSec[0] =
            (pool.allocPoint * mevPerSec * lpPercent) /
            totalAllocPoint /
            total;
    ...
    // View function to see if user can harvest Mev.
    function canHarvest(
        uint256 _pid,
        address _user
    ...
    function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
        ...
        uint256 mevReward = ((multiplier * mevPerSec) * pool.allocPoint) /
            totalAllocPoint;
        ...
        if (marketingPercent > 0) {
            mev.mint(
                marketingAddress,
                (mevReward * marketingPercent) / total
            );
        }

        if (teamPercent > 0) {
            mev.mint(teamAddress, (mevReward * teamPercent) / total);
        }

        mev.mint(address(this), (mevReward * lpPercent) / total);

        pool.accMevPerShare +=
            (mevReward * ACC_TOKEN_PRECISION * lpPercent) /
            pool.totalLp /
            total;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        IMevdexPair pair = IMevdexPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount);
    }

    // Deposit tokens for Mev allocation.
    ...
    function _deposit(
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(mev)) {
                totalMevInPools += _amount;
            }
        }
        user.rewardDebt =
            (user.amount * pool.accMevPerShare) /
            ACC_TOKEN_PRECISION;
        ...
        for (
            ...
            pool.rewarders[rewarderId].onMevReward(
                _pid,
                msg.sender,
                user.amount
            );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        for (
            ...
            pool.rewarders[rewarderId].onMevReward(_pid, msg.sender, 0);
        }

        if (address(pool.lpToken) == address(mev)) {
            totalMevInPools -= amount;
        }
    ...
    // Pay or lockup pending Mev.
    function payOrLockupPendingMev(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = ((user.amount * pool.accMevPerShare) /
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.rewardLockedUp = 0;
                user.nextHarvestUntil = block.timestamp + pool.harvestInterval;
                safeMevTransfer(msg.sender, pendingRewards);
            }
        } else if (pending > 0) {
    ...
    function safeMevTransfer(address _to, uint256 _amount) internal {
        if (mev.balanceOf(address(this)) > totalMevInPools) {
            uint256 mevBal = mev.balanceOf(address(this)) -
                totalMevInPools;
            if (_amount >= mevBal) {
                mev.safeTransfer(_to, mevBal);
            } else if (_amount > 0) {
                mev.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _mevPerSec) public onlyOwner {
        _massUpdatePools();

        emit EmissionRateUpdated(msg.sender, mevPerSec, _mevPerSec);
    ...
    function getMevPerSec() public view returns (uint256) {
        return mevPerSec;
    }
}